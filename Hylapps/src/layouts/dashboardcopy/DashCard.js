...
import { Button, Dialog, DialogTitle, DialogContent, DialogActions, Grid, Icon,Tooltip } from "@mui/material";
...
<Tooltip
    title={showAddButton ? "" : "Vessel limit exceeded. Delete vessels to add new."}
    arrow
  >
    <span> {/* Wrap the Button in a span because a disabled Button does not render Tooltip */}
      <Button
        variant="contained"
        color="primary"
        startIcon={<Icon>add</Icon>}
        style={{
          backgroundColor: showAddButton ? "#0F67B1" : "#BDBDBD", // Change color when disabled
          color: "white",
          borderRadius: "5px",
          padding: "6px 12px",
          cursor: showAddButton ? "pointer" : "not-allowed",
        }}
        onClick={showAddButton ? handleToggleSearchBar : null}
        disabled={!showAddButton} // Disable when limit is exceeded
      >
        Add Vessel
      </Button>
    </span>
