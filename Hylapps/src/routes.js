...
const allRoutes = [
  ...
  {
    type: "route",
    name: "SGS NL Dashboard",
    key: "sgsnl_dashboard",
    route: "/sgsnl_dashboard",
...
const getFilteredRoutes = (role) => {
  if (role === "hyla admin") {
    ...
    return allRoutes.filter((route) => ["sign-in" ,"reset-password" , "sgsnl_dashboard", "dashboard", "Geofence" , "alerts", "create-users","ShipRADAR"].includes(route.key));
  } else if (role === "organizational user") {
    // Return only specific routes for Organization User
    return allRoutes.filter((route) => ["sign-in" ,"reset-password" ,"sgsnl_dashboard", "dashboard","ShipRADAR"].includes(route.key));
  } 
  else if (role === "guest") {
    // Return empty array or a default route for guest users
    return allRoutes.filter((route) => ["sign-in" ,"reset-password" , "sgsnl_dashboard", "dashboard"].includes(route.key));
  } 
